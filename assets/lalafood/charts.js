...
(() => {
  ...
  function heat(c,rows,cols,lookup,options={}) {
    const cellInk=background=>{
      const {r,g,b}=d3.rgb(background);
      const [lr,lg,lb]=[r,g,b].map(v=>{v/=255;return v<=.04045?v/12.92:((v+.055)/1.055)**2.4;});
      return .2126*lr+.7152*lg+.0722*lb>.179?'#000':'#fff';
    };
    ...
    for(const row of rows){const tr=body.insertRow(),th=document.createElement('th');th.scope='row';th.textContent=options.rowLabel?options.rowLabel(row):row;tr.append(th);
      for(const col of cols){const td=tr.insertCell(),d=lookup(row,col);if(!d){td.textContent='—';td.setAttribute('aria-label','Not observed');continue;}
        const v=d.value,t=Math.min(1,v/(options.max||35));td.style.background=d3.interpolateRgb(C.wash,C.accent)(t);td.style.color=cellInk(td.style.background);
  ...
  const renderers={
    ...
    discovery(c,s){
      if(s.view==='ratings'){
        ...
        svg.append('path').datum(D.distance).attr('d',d3.area().x(d=>x(d.label)).y0(h-m.b).y1(d=>y(d.rate))).attr('fill',C.selection);
        svg.append('path').datum(D.distance).attr('d',d3.line().x(d=>x(d.label)).y(d=>y(d.rate))).attr('stroke',C.accent).attr('stroke-width',3).attr('fill','none');
        const dots=svg.selectAll('circle').data(D.distance).join('circle').attr('cx',d=>x(d.label)).attr('cy',d=>y(d.rate)).attr('r',7).attr('fill',C.accent).attr('stroke','white').attr('stroke-width',2);
    ...
    retention(c,s){
      if(s.view==='acquisition'){
        ...
        for(const [key,values] of groups)svg.append('path').datum(values).attr('d',line).attr('stroke',key===s.cohort?C.accent:C.faint).attr('stroke-width',key===s.cohort?3:1.4).attr('fill','none');
        const rows=groups.get(s.cohort),marks=svg.selectAll('circle').data(rows).join('circle').attr('cx',d=>x(d.age)).attr('cy',d=>y(d.rate)).attr('r',5).attr('fill',C.accent).attr('stroke','white').attr('stroke-width',1.5);
    ...
    conclusion(c,s){
      ...
      const dots=svg.selectAll('circle').data(phases).join('circle').attr('cx',d=>x(d[0])).attr('cy',30).attr('r',12).attr('fill',d=>d[0]===s.phase?C.accent:C.faint);interactive(dots,c,d=>d[1],d=>{s.phase=d[0];c.redraw();});
