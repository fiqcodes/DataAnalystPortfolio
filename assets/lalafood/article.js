...
(() => {
  ...
  function activate(id){if(id===active)return;active=id;steps.forEach(e=>e.classList.toggle('is-active',e.dataset.viz===id));if(mobile())ensureMobile(steps.find(e=>e.dataset.viz===id));else createContext(desktop,id);}
  function sync(){framePending=false;const pivot=innerHeight*.42;let current=steps[0];for(const step of steps)if(step.getBoundingClientRect().top<=pivot)current=step;activate(current.dataset.viz);}
  ...
  document.querySelectorAll('a[href^="#"]').forEach(a=>a.addEventListener('click',e=>{const target=document.getElementById(a.hash.slice(1));if(!target)return;e.preventDefault();history.pushState(null,'',a.hash);target.scrollIntoView({behavior:reduced?'instant':'smooth'});target.tabIndex=-1;target.focus({preventScroll:true});}));
